...
export default function EditProductForm({ product }: { product: Product }) {
  ...
  // Parse incoming JSON
  const initialSizes = JSON.parse(product.sizes || '[]') as string[];
  const [selectedSizes, setSelectedSizes] = useState<string[]>(initialSizes);
  
  // Image handling
  const initialImages = JSON.parse(product.images || '[]') as string[];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      productData.append('images', JSON.stringify(finalImageUrls));

      const res = await editProduct(product.id, productData);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-serif font-bold text-zinc-900">Edit Product</h1>
          <p className="text-zinc-600">Update details for {product.title}.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-zinc-100">
          <label className="block text-sm font-medium text-zinc-700 mb-4">Product Images ({imagePreviews.length} uploaded)</label>
